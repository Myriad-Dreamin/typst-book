...
use typst_ts_svg_exporter::{
    flat_ir::{serialize_doc, LayoutRegionNode, Page, PageMetadata},
    Module, ModuleBuilder, MultiSvgDocument,
};
...
impl TypstRenderer {
    ...
    pub fn compile_page_with(&mut self, path: &Path, settings: CompilePageSetting) -> ZResult<()> {
        self.setup_entry(path);
